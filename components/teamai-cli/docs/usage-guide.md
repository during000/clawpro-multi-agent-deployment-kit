...
| `{baseUrl}/api/local-agent/wake-ticket` | POST | Mint a short-lived single-use WebSocket ticket |
| `{baseUrl}/api/local-agent/wake` | WebSocket | Receive lightweight `task_available` wake events |

Run `teamai agent-task-listen --tool codebuddy` to keep an outbound wake connection. The WebSocket carries only task metadata; TeamAI claims full commands through authenticated HTTPS sync and automatically performs a recovery sync after reconnecting.
...
The backend may push an **`execute_agent_task`** command to run a task in an existing
CodeBuddy workspace:
...
```json
{
  ...
  "prompt": "Fix the login error and add a regression test"
}
```

TeamAI validates the command against its local workspace-to-project binding, creates a
per-task deduplication lock, and starts a detached worker so the host hook is never blocked by
the Agent run. The worker drives CodeBuddy over ACP JSON-RPC stdio, acknowledges `running`
with complete output snapshots, and finally acknowledges `success` or `failed` with the ACP
session ID. Only CodeBuddy is enabled in this initial implementation. The default tool set is
`Read,Write,Edit,Glob,Grep`; adding `Bash` requires an explicit local opt-in through
`TEAMAI_AGENT_TASK_TOOLS`.
...
| `TEAMAI_CODEBUDDY_PATH` | Absolute path to the CodeBuddy executable when it is not discoverable from `PATH` or the WorkBuddy application bundle |
| `TEAMAI_AGENT_TASK_TOOLS` | Comma-separated CodeBuddy tools allowed for remote Agent tasks (default `Read,Write,Edit,Glob,Grep`) |
| `TEAMAI_AGENT_TASK_TIMEOUT_MS` | Agent task timeout in milliseconds (default 20 minutes, maximum 60 minutes) |
| `TEAMAI_LOCAL_AGENT_HOME` | Override the local-agent state directory; intended for isolated development and end-to-end tests |
