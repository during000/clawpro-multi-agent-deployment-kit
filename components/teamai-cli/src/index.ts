...
sourceCmd
  .command('execute-agent-task', { hidden: true })
  .description('Run one detached ClawPro Agent task (internal)')
  .action(async () => {
    const { runAgentTaskWorkerFromStdin } = await import('./local-agent.js');
    await runAgentTaskWorkerFromStdin();
  });
...
program
  .command('agent-task-listen')
  .description('Keep an outbound WebSocket connection to ClawPro and claim Agent tasks over HTTPS')
  .option('--tool <name>', 'Local Agent type used for task routing', 'codebuddy')
  .option('--cwd <path>', 'Workspace used for report/sync', process.cwd())
  .option('--once', 'Connect once and exit after the connection closes')
  .action(async (cmdOpts: { tool?: string; cwd?: string; once?: boolean }) => {
    const { runLocalAgentWakeListener } = await import('./local-agent-listener.js');
    await runLocalAgentWakeListener(cmdOpts);
  });

program
  .command('iwiki-mcp-proxy', { hidden: true })
  .description('Internal read-only iWiki MCP bridge for TeamAI-managed Agent tasks')
  .action(async () => {
    const { runIWikiMcpProxy } = await import('./iwiki-mcp-proxy.js');
    await runIWikiMcpProxy();
  });
