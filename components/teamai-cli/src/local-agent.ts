...
import type { AgentTaskSpec } from './agent-task-executor.js';
...
const CLAWPRO_ARTIFACT_BUNDLE_START = '<clawpro_artifact_bundle_v1>';
const CLAWPRO_ARTIFACT_BUNDLE_END = '</clawpro_artifact_bundle_v1>';
const MAX_AGENT_TASK_ARTIFACT_BYTES = 1_200_000;
...
export const DEFAULT_ROUTES = {
  ...
  wakeTicket: '/api/local-agent/wake-ticket',
  wake: '/api/local-agent/wake',
...
function getLocalAgentHome(): string {
  const override = process.env.TEAMAI_LOCAL_AGENT_HOME?.trim();
...
function getAgentTaskWorkerDir(): string {
  return path.join(getLocalAgentHome(), 'agent-tasks');
}

function getAgentTaskWorkerLockPath(taskId: number): string {
  return path.join(getAgentTaskWorkerDir(), `${taskId}.json`);
}

async function isWorkerLockActive(lockPath: string): Promise<boolean> {
  try {
...
async function validateAgentTaskCommand(
  config: LocalAgentConfig,
  command: LocalAgentCommand,
  context: LocalAgentContext,
): Promise<AgentTaskSpec> {
  const agentType = normalizeAgentType(command.agent_type ?? context.tool ?? '');
  const currentTool = normalizeAgentType(context.tool ?? '');
  if (agentType !== 'codebuddy' || currentTool !== agentType) {
    throw new Error(`Agent task runtime mismatch: expected ${currentTool || 'unknown'}, got ${agentType || 'unknown'}`);
  }
  if (!command.workspace_path) throw new Error('Agent task is missing workspace_path');
  if (!command.prompt?.trim()) throw new Error('Agent task is missing prompt');

  const workspacePath = await canonicalizeWorkspacePath(command.workspace_path);
  const binding = config.workspaceBindings[workspacePath];
  if (!binding || binding.projectId <= 0) {
    throw new Error('Agent task workspace is not bound to a ClawPro project');
  }
  if (command.project_id && binding.projectId !== command.project_id) {
    throw new Error('Agent task project does not match the local workspace binding');
  }
  if (binding.ideType && normalizeAgentType(binding.ideType) !== agentType) {
    throw new Error('Agent task runtime does not own the local workspace binding');
  }
  ...
  return {
    ...
    executor: command.executor === 'imate' ? 'imate' : 'codebuddy',
    targetAgentId: command.target_agent_id,
    imateProjectId: command.imate_project_id,
  };
}

/**
 * Spawn long-running CodeBuddy work outside the host hook process. A per-task
 * lock prevents duplicate execution while the backend still returns the task
 * as pending. Dead worker locks are automatically reclaimed on the next sync.
 */
async function queueAgentTaskCommand(
  config: LocalAgentConfig,
  command: LocalAgentCommand,
  context: LocalAgentContext,
): Promise<boolean> {
  await validateAgentTaskCommand(config, command, context);
  const entry = resolveCmdEntry();
  if (!entry) throw new Error('Cannot resolve teamai entry script for agent task worker');

  const workerDir = getAgentTaskWorkerDir();
  const lockPath = getAgentTaskWorkerLockPath(command.id);
  await ensureDir(workerDir);
  if (await pathExists(lockPath)) {
    if (await isWorkerLockActive(lockPath)) return false;
    await fs.promises.rm(lockPath, { force: true });
  }
  ...
  try {
    ...
    // A WSS wake and the resident polling loop may observe the same command at
    // nearly the same instant. The exclusive open is the real atomic lock;
    // losing that race means another worker owns this task, not task failure.
    if ((error as NodeJS.ErrnoException).code === 'EEXIST') return false;
    throw error;
  }
  await handle.writeFile(JSON.stringify({ createdAt: Date.now() }), 'utf8');
  await handle.close();
  ...
  try {
    ...
    const child = spawn(
      process.execPath,
      [entry, 'source', 'execute-agent-task'],
      {
        detached: true,
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.stdin.end(JSON.stringify({ command, context }), () => resolve());
    });
    child.unref();
    return true;
  } catch (error) {
...
async function readWorkerInput(): Promise<{ command: LocalAgentCommand; context: LocalAgentContext }> {
  let raw = '';
  process.stdin.setEncoding('utf8');
  for await (const chunk of process.stdin) raw += chunk;
  if (!raw.trim()) throw new Error('Agent task worker received empty input');
  return JSON.parse(raw) as { command: LocalAgentCommand; context: LocalAgentContext };
}

function declaredArtifactPaths(prompt: string): string[] {
  ...
  const inline = prompt.match(/必须返回的产物：\s*([^\n]+)/)?.[1];
...
async function workflowTaskSlug(workspacePath: string, prompt: string): Promise<string> {
  try {
    const state = JSON.parse(
      await fs.promises.readFile(path.join(workspacePath, 'workflow-state.json'), 'utf8'),
    ) as { task_slug?: unknown };
    if (typeof state.task_slug === 'string' && state.task_slug.trim()) {
      return state.task_slug.trim();
    }
  } catch {
    // The first node can run before workflow-state.json exists.
  }
  const match = prompt.match(/"task_slug"\s*:\s*\{[\s\S]*?"value"\s*:\s*"([^"]+)"/);
  return match?.[1]?.trim() ?? '';
}

async function resolveProducedArtifact(
  workspacePath: string,
  relativePath: string,
  taskSlug: string,
): Promise<string | null> {
  ...
  const candidates = [
    ...
    ...(taskSlug
      ? [
        path.join(workspacePath, 'server', 'artifacts', taskSlug, normalized),
        path.join(workspacePath, 'artifacts', taskSlug, normalized),
      ]
      : []),
  ];
  for (const candidate of candidates) {
    try {
      ...
      // Try the next contract-compatible location.
    }
  }

  const suffix = `/${normalized}`;
  const matches: Array<{ path: string; mtimeMs: number }> = [];
  for (const file of await listFilesRecursive(workspacePath)) {
    const portable = file.replaceAll('\\', '/');
    if (portable !== normalized && !portable.endsWith(suffix)) continue;
    const absolute = path.join(workspacePath, file);
    const stat = await fs.promises.stat(absolute);
    if (stat.isFile() && stat.size > 0) matches.push({ path: absolute, mtimeMs: stat.mtimeMs });
  }
  matches.sort((left, right) => right.mtimeMs - left.mtimeMs);
  return matches[0]?.path ?? null;
}

export async function appendClawProArtifactBundle(
  result: string,
  spec: AgentTaskSpec,
): Promise<string> {
  const required = declaredArtifactPaths(spec.prompt);
  if (required.length === 0) return result;
  const taskSlug = await workflowTaskSlug(spec.workspacePath, spec.prompt);
  const artifacts: Array<Record<string, unknown>> = [];
  let totalBytes = 0;
  for (const relativePath of required) {
    const producedPath = await resolveProducedArtifact(
      spec.workspacePath,
      relativePath,
      taskSlug,
    );
    if (!producedPath) continue;
    const raw = await fs.promises.readFile(producedPath);
    if (totalBytes + raw.length > MAX_AGENT_TASK_ARTIFACT_BYTES) {
      throw new Error('ClawPro node artifacts exceed the TeamAI upload limit');
    }
    totalBytes += raw.length;
    artifacts.push({
      path: relativePath,
      source_path: path.relative(spec.workspacePath, producedPath).replaceAll('\\', '/'),
      size: raw.length,
  ...
  return `${result.trim()}\n\n${CLAWPRO_ARTIFACT_BUNDLE_START}\n${JSON.stringify(bundle)}\n${CLAWPRO_ARTIFACT_BUNDLE_END}`;
}

export async function runAgentTaskWorker(
  command: LocalAgentCommand,
  context: LocalAgentContext,
): Promise<void> {
  let config: LocalAgentConfig | null = null;
  try {
    config = await loadLocalAgentConfig();
    if (!config) throw new Error('Local Agent configuration is missing');
    const spec = await validateAgentTaskCommand(config, command, context);
    const tag = localAgentTag(context);
    await ackCommand(config, tag, command, 'running', undefined, undefined, 'TeamAI accepted the task.');

    const { executeCodeBuddyAgentTask, executeIMateAgentTask } = await import('./agent-task-executor.js');
    const execute = spec.executor === 'imate' ? executeIMateAgentTask : executeCodeBuddyAgentTask;
    const execution = await execute(spec, async (progress) => {
      await ackCommand(
        config!,
        tag,
        command!,
    ...
    const resultWithArtifacts = await appendClawProArtifactBundle(execution.result, spec);
    await ackCommand(
      config,
      tag,
      command,
    ...
    if (config) {
      try {
        await ackCommand(config, localAgentTag(context), command, 'failed', undefined, message, '', undefined);
      } catch (ackError) {
        log.debug(`[local-agent] failed to ack agent task failure: ${(ackError as Error).message}`);
      }
    }
    throw error;
  } finally {
    if (command.id) {
      await fs.promises.rm(getAgentTaskWorkerLockPath(command.id), { force: true });
    }
  }
}

/** Hidden detached-worker entrypoint used by `source execute-agent-task`. */
export async function runAgentTaskWorkerFromStdin(): Promise<void> {
  const { command, context } = await readWorkerInput();
  await runAgentTaskWorker(command, context);
}
...
async function processCommands(
  ...
  for (const command of commands) {
    ...
    try {
      if (command.type === 'execute_agent_task') {
        const queued = await queueAgentTaskCommand(config, command, context);
        log.debug(`${tag} agent task ${command.id} ${queued ? 'queued' : 'already running'}`);
        continue;
      }
