...
export function PolicyDetailDrawer({
  ...
  return (
    <Drawer open={detailVisible} onOpenChange={open => { if (!open) setDetailVisible?.(false); }} direction="right">
          ...
          <div className="flex-1 min-w-0 space-y-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {String(selectItem?.PolicyType) === '1' && (
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button variant="claw-outline" size="claw-sm">{'删除'}</Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent className="sm:max-w-[420px]">
                    <AlertDialogHeader>
              ...
              <div className="rounded-[4px] border border-[#E5E5E5] overflow-hidden divide-y divide-[#E5E5E5]">
                <div className="grid grid-cols-2 divide-x divide-[#E5E5E5]">
                  <div className="flex items-center gap-3 px-3 py-2.5">
                    ...
                    <BodyText as="span" className="flex-1 min-w-0 truncate" title={selectItem?.PolicyName}>
              ...
              <div className="rounded-[4px] border border-[#E5E5E5] overflow-hidden divide-y divide-[#E5E5E5]">
                <div className="flex items-center gap-3 px-3 py-2.5">
                  ...
                  <span className={`text-sm ${
                    String(selectItem?.PolicyAction) === '1' ? 'text-[#16A34A]' : 'text-[#DC2626]'
                  }`}>
                    {String(selectItem?.PolicyAction) === '1' ? '白名单' : '黑名单'}
                  </span>
                </div>
                <div className="flex items-center gap-3 px-3 py-2.5">
                  <MetaText className="w-[88px] flex-shrink-0">执行动作</MetaText>
                  {(() => {
                    const action = String(selectItem?.PolicyAction);
                    const color = action === '2' ? '#DC2626'
                      : action === '0' ? '#D97706'
                        : action === '1' ? '#16A34A'
                          : '#525252';
                    return (
                ...
                <div className="flex items-start gap-3 px-3 py-2.5">
                  <MetaText className="w-[88px] flex-shrink-0">域名详情</MetaText>
                  <div className="flex-1 min-w-0 break-all">
                    <CodeText tone="emphasis">{selectItem?.Domains?.join?.('；') || '--'}</CodeText>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              {String(selectItem?.HostScope) !== '0' ? (
                <div className="rounded-[4px] border border-[#E5E5E5] px-3 py-2.5 text-sm text-[#0A0A0A]">
                  {GetHostTypeText(selectItem?.HostScope)}
                </div>
              ) : (
                <div className="relative rounded-[4px] border border-[#E5E5E5] overflow-hidden bg-white">
                  ...
                  <div className="grid grid-cols-[1fr_auto] items-center gap-4 px-4 py-2 border-t border-[#f0f0f0]">
                    <span className="justify-self-start text-sm leading-[1.5] text-[#737373]">
                      共 {selectItem?.HostIds?.length || 0} 条记录
                    </span>
                    <Pagination
