 ...
 *
 * AuditLog（操作记录页）专用：停服态下让本页两个 "开始日期 / 结束日期" DatePicker
 * 弹出的日历面板 100% 正常可用（不透明 / 正常配色 / 正常点击 / 正常聚焦）。
 *
 * 背景与方案与 OpenClawMonitor / OpsObservation / SessionManagement / TokensMonitor
 * 页同款（详见
 ...
 *     保证"停服前已禁用则延续禁用"。
 *
 * 本页触发器外层 <div className="flex items-center gap-2" data-billing-exempt>
 * 已经打过标（页面此前对触发器视觉/事件放行做过处理），但缺少
 * "面板挂到 body 外层"的四重兜底，导致弹出后仍被 overlay 拦截。
 *
 * 四重兜底
 * ─────────────────────────────────────────────
 ...
 * 约束
 * ─────────────────────────────────────────────
 * · 只修改本页范围内逻辑：hook 文件位于 pages/admin/AuditLog/ 子目录；
 *   在 AuditLog.tsx 中调用。
...
const STYLE_ID = "audit-log-calendar-billing-exempt-style";
...
function ensureSupplementaryStyle(): void {
  ...
  style.id = STYLE_ID;
  style.textContent = `
/* AuditLog 页 DatePicker 日历面板停服态豁免（页面级补充规则）
...
/**
 * 遍历 root 范围内所有popper wrapper，若其内含 Calendar，则依 markWrapper 打标。
 *
...
export function useAuditLogCalendarBillingExempt(): void {
  useEffect(() => {
    if (typeof document === "undefined") return;
...
export default useAuditLogCalendarBillingExempt;
