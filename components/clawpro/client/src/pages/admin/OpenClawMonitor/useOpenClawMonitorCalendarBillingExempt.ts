 ...
 *
 * OpenClawMonitor（Agent 列表页）专用：停服态下让本页两个"选择日期" DatePicker
 * 弹出的日历面板 100% 正常可用（不透明 / 正常配色 / 正常点击 / 正常聚焦）。
 *
 * 背景
 * ─────────────────────────────────────────────
 * · 页面 JSX 里两个 <DatePicker/> 外层已经用<div data-billing-exempt> 包裹（约 L2975），
 *   触发器（"选择日期"按钮）本身在停服态下可点、可打开面板。
 * · 但 DatePicker 底层是 Radix Popover：日历面板走 PopoverContent Portal 到 <body> 下，
 *   与触发器在真实 DOM 里是兄弟树。AdminDisabledOverlay：
 *     - 文档层 capture 事件拦截：target.closest('[data-billing-exempt]') 走真实 DOM 父链；
 *     - CSS灰化选择器：body.admin-billing-suspended [data-radix-popper-content-wrapper]
 *       button / a / input / [role=switch] / [role=checkbox] / [role=combobox] /
 *       [role=menuitem] / [role=option] { opacity: 0.4; cursor: not-allowed }
 *   所以触发器外层的 data-billing-exempt 覆盖不到日历面板。
 * · 已有全局 hook useDatePickerBillingExempt 尝试给 wrapper 自身打 exempt，但 CSS 里
 *   popper-wrapper 类只有"后代恢复分支"（[wrapper] [data-billing-exempt] *），
 *   没有"同元素恢复分支"，所以 wrapper 自身打的 exempt 恢复不了内部 button 视觉状态。
 *
 * 方案（四重兜底，稳定优先）
 * ─────────────────────────────────────────────
 * 每次日历Popover 挂载时对本次面板做以下事情，任一条命中即恢复正常态：
 *   (a) 在 wrapper、popover-content、Calendar 根节点上都打 data-billing-exempt——
 *       为 AdminDisabledOverlay 的 JS 事件拦截提供多层放行标记（closest 无论从哪级
 *       DOM 起冒泡都能找到 exempt）。
 *   (b) 在 Calendar 根节点上打 data-billing-exempt——命中 overlay CSS 的
 *       "[wrapper] [data-billing-exempt] *" 恢复分支，把内部所有 button / span 等
 *       后代恢复到 opacity:1、cursor:pointer。
 *   (c) 注入一段作用域为 body.admin-billing-suspended 的补充 style（仅一次，全局共享），
 *       用更高特异性+ !important 显式声明 popper-wrapper 内含 calendar 时其所有
 *       button恢复正常态——作为 CSS 优先级/顺序层面的兜底，防止 (b) 因特异性问题失效。
 *   (d) 事件同步兜底：在 document capture 阶段（早于 overlay 的 capture 监听）
 *       监听 pointerdown/mousedown/click，若 target 落在含 Calendar 的
 *       popper-wrapper 内、且 wrapper 尚未打标，就地同步打标——这样即使
 *       MutationObserver 因时序异步尚未回调，overlay 后续用closest 也一定命中。
 *
 * "停服前已禁用则延续禁用"约束
 * ─────────────────────────────────────────────
 * · Hook 只添加 data属性和 CSS 恢复样式；从不触碰任何元素的 disabled / aria-disabled /
 *   pointer-events / 原生表单 disabled 语义。
 * · Calendar 内部由 DatePicker 传入的 min/max/disabledMatcher 生成的不可选日期，
 *   react-day-picker 会给这些日期按钮渲染 aria-disabled + disabled + .rdp-day_disabled；
 *   补充 style 用 :not([disabled]):not([aria-disabled="true"]):not(.rdp-day_disabled)
 *   排除这些"业务侧本就应禁用"的日期，确保它们照旧禁用。
 * · 若 DatePicker 自身 disabled，Popover 不会打开，面板不出现，与本 hook 无关。
 *
 * 约束
 * ─────────────────────────────────────────────
 * · 只修改本页范围内逻辑：hook 文件位于 pages/admin/OpenClawMonitor/ 子目录；
 *   在 OpenClawMonitor.tsx 中调用。
...
const STYLE_ID = "openclaw-monitor-calendar-billing-exempt-style";
...
function ensureSupplementaryStyle(): void {
  ...
  style.id = STYLE_ID;
  style.textContent = `
/* OpenClawMonitor 页DatePicker 日历面板停服态豁免（页面级补充规则）
 * 说明：与 AdminDisabledOverlay 里"popper-wrapper button 灰化"配对；
 * 通过 :has([data-slot="calendar"]) 锁定"只是日历面板"、不影响其它 Popover。
 * 排除条件保证"停服前已禁用则延续禁用"：react-day-picker 用 disabled 属性 +
 * aria-disabled + .rdp-day_disabled 三重语义标记不可选日期，任一命中都不恢复。 */
...
/**
 * 遍历 root 范围内所有 popper wrapper，若其内含 Calendar，则依markWrapper 打标。
 *
 ...
 * 导致 overlay 的 click拦截把用户点击当作管辖范围事件而弹出 toast。
 */
...
export function useOpenClawMonitorCalendarBillingExempt(): void {
  useEffect(() => {
    if (typeof document === "undefined") return;
    ...
    // (a)(b) 初次扫描 + MutationObserver：不硬依赖 body class（比如页面刚挂时
    // AdminDisabledOverlay 的 effect 时序），无条件打标——非停服态下 exempt 属性
    // 存在也无副作用（overlay 的规则只在 admin-billing-suspended 时激活）。
    ...
    // AdminDisabledOverlay 的监听点），先于 overlay 就地给wrapper 打标——
...
export default useOpenClawMonitorCalendarBillingExempt;
