 ...
 *
 * SessionManagement（会话管理页）专用：停服态下让本页 "选择日期" DatePicker
 * 弹出的日历面板 100% 正常可用（不透明 / 正常配色 / 正常点击 / 正常聚焦）。
 *
 * 背景与方案与 OpenClawMonitor / OpsObservation / TokensMonitor 页同款
 * （详见 pages/admin/OpenClawMonitor/useOpenClawMonitorCalendarBillingExempt.ts
 * 头部注释）：
...
const STYLE_ID = "session-management-calendar-billing-exempt-style";
...
function ensureSupplementaryStyle(): void {
  ...
  style.id = STYLE_ID;
  style.textContent = `
/* SessionManagement 页 DatePicker 日历面板停服态豁免（页面级补充规则）
...
export function useSessionManagementCalendarBillingExempt(): void {
  useEffect(() => {
    if (typeof document === "undefined") return;
...
export default useSessionManagementCalendarBillingExempt;
