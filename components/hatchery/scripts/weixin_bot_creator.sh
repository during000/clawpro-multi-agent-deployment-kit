...
# 配置微信通道前，先将插件 ID 加入 plugins.allow，再 enable 插件
# 检查 openclaw-weixin 插件是否已经启用
PLUGIN_ENABLED=false
echo "检查 openclaw-weixin 插件状态..."
if command -v openclaw >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    _plugin_info=$(
        set +o pipefail
        openclaw plugins list --json 2>/dev/null \
            | jq -r '.. | objects | select((.id // .name // "") == "openclaw-weixin") | "\(.enabled // "")|\(.status // .state // "")"' 2>/dev/null \
            | head -n 1
    ) || true
    ...
    if [ "$_plugin_enabled_field" = "true" ] || [ "$_plugin_status" = "loaded" ] || [ "$_plugin_status" = "enabled" ]; then
        PLUGIN_ENABLED=true
        echo "✓ 检测到 openclaw-weixin 插件已启用 (enabled=${_plugin_enabled_field}, status=${_plugin_status})，跳过插件配置步骤"
    elif [ -n "$_plugin_status" ] || [ -n "$_plugin_enabled_field" ]; then
        echo "  openclaw-weixin 插件当前 enabled=${_plugin_enabled_field}, status=${_plugin_status}，需要启用"
    else
        echo "  未从 JSON 输出中匹配到 openclaw-weixin 插件，需要启用"
    fi
fi
...
def resolve_plugin_dir() -> Path:
    ...
    优先级:
      1. 新版 npm/projects 路径
      2. 新版 npm/node_modules 路径
      3. 旧版 extensions 路径
    若都不存在，返回首个候选作为占位（后续 detect_plugin_version 会判定缺失）。
    """
    for candidate in _PLUGIN_DIR_CANDIDATES:
        if (candidate / "package.json").is_file():
            return candidate

    # 都没找到，返回最优候选（新版路径）作为占位，后续会因 package.json 缺失而报错
    return _PLUGIN_DIR_CANDIDATES[0]
...
def cmd_create():
    ...
    if plugin_version == "unknown":
        searched = " | ".join(str(p) for p in _PLUGIN_DIR_CANDIDATES)
        _emit_error(
            "init",
            f"未找到微信插件，请先安装 openclaw-weixin。已查找路径: {searched}",
        )
        sys.exit(1)
    _log_info("init", f"微信插件路径: {PLUGIN_DIR}")
