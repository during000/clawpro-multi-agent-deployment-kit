...
# batch_install_skills_from_smh_hermes.sh
# 从 SMH 批量下载技能 zip 包并安装到 Hermes 实例
#
...
#   - 不强制校验 SKILL.md（hermes 生态对包结构更宽松）
#   - 安装完成后尝试 harness skills reload（非关键路径）
...
echo "Batch installing ${SKILL_COUNT} skill(s) to Hermes..."
...
install_single_skill() {
    ...
    local lock_file="/tmp/install-skill-hermes-${skill_slug}.lock"
    local tmp_zip=""
    local staging_dir=""
    local backup_dir=""
...
# ===== Hermes 特有：尝试 reload =====
# harness skills reload 让 gateway 立即加载新技能（非关键路径，失败不影响结果）
if command -v harness >/dev/null 2>&1; then
    echo ""
    echo "Triggering harness skills reload..."
    harness skills reload >/dev/null 2>&1 && echo "✓ harness skills reload triggered" || \
        echo "ℹ harness skills reload not available; relying on watcher/next session"
fi
