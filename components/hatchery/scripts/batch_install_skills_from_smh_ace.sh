...
# batch_install_skills_from_smh_ace.sh
# 从 SMH 批量下载技能 zip 包并安装到 LightClaw-ACE 实例
#
# 参数（通过 TAT 模板变量注入，与 batch_install_skills_from_smh.sh 契约完全一致）：
#   {{skills_list}} - base64 编码的多行文本，每行一个技能，字段用 TAB 分隔：
#     download_url<TAB>skill_slug<TAB>skill_version
#
# 输出：最后一行为 JSON 格式的安装结果汇总（方便服务端解析）
#
# 与 openclaw batch_install_skills_from_smh.sh 的差异：
#   - 安装目录: ~/.openclaw/workspace/skills → ~/.lightclaw/workspace/skills
#   - 依赖多 jq（写 lightclaw.json 启用技能）
#   - 不强制校验 SKILL.md（ACE 生态对包结构更宽松）
#   - 安装完成后写入 lightclaw.json 启用技能 + systemctl restart lightclaw
#   - 其余安全防护完全对齐：flock / Zip Slip / 200MB 上限 / 磁盘预检 / 备份回滚
...
ensure_dependencies() {
    ...
    if [ "${need_install}" = true ]; then
        ...
        $SUDO_CMD yum install -y -q curl unzip jq 2>/dev/null || {
            echo "Failed to install dependencies (curl, unzip, jq)" >&2
            exit 1
        }
    fi
}
...
enable_skill_in_config() {
    ...
    if jq --arg s "$slug" \
        ...
        echo "warn: jq failed to update ${CONFIG}" >&2
        return 1
    fi
}
...
echo "Batch installing ${SKILL_COUNT} skill(s) to ACE..."

# 检查 /tmp 磁盘空间
TMP_AVAIL_KB=$(available_kb /tmp)
REQUIRED_TMP_KB=$((SKILL_COUNT * 65536))
...
install_single_skill() {
    ...
    local lock_file="/tmp/install-skill-ace-${skill_slug}.lock"
    local tmp_zip=""
    local staging_dir=""
    local backup_dir=""
    ...
    # ACE 特有：写入 lightclaw.json 启用该技能
    if ! enable_skill_in_config "${skill_slug}"; then
        # config 写入失败不回滚文件（技能文件已就位，下次启动会自动加载）
        echo "[${skill_slug}] warn: enable in config failed, skill files are ready" >&2
    fi
...
# ===== ACE 特有：重启 lightclaw =====
echo ""
echo "Restarting lightclaw..."
systemctl restart lightclaw 2>&1 || echo "warn: lightclaw restart failed (可能是权限或未安装)" >&2
