#!/bin/bash
# detect_openclaw_install.sh — 检测 openclaw 装在哪个用户下
#
# 上层 agent_checker.go 只读 runtime_user / runtime_home 两个字段，
# 故脚本只做这一件事：
#   1) 当前用户下有 openclaw → 当前用户
#   2) 否则扫 /home/* 和 /root，找到第一个装了 openclaw 的用户
#   3) 都没找到 → 返回当前用户（OpenClaw 分支不走 agentuser 兜底，空值由上层决策）
#
# 为避免机器刚启动时外部命令 hang：不调用 openclaw/systemctl，只做文件系统检查。
# 脚本始终 exit 0，失败语义体现在 runtime_user 的取值上。
set -u

# ========== 日志系统初始化 ==========
# 重要：调用方（Go 后端 detectAndSaveRuntimeUser / HandleVersionInfo / admin_instances 探测接口）
...
SCRIPT_NAME="detect_openclaw_install"
...
# 脚本正常路径最后会把 _JSON_EMITTED 置 1 并 trap - EXIT，故正常场景不会触发此兜底。
_JSON_EMITTED=0
_emit_fallback_json() {
  [ "$_JSON_EMITTED" = "1" ] && return 0
  # 尽力用已有变量，若变量未定义则回退为空串（仍保证 JSON 合法）
...
echo "=== detect_openclaw_install 开始 ==="

# 判断给定 HOME 下是否已安装 openclaw
has_openclaw() {
    local h="$1"
    [ -d "$h/.openclaw" ]           && return 0
    [ -x "$h/.local/bin/openclaw" ] && return 0
    return 1
}
...
# 1) 优先：当前用户自己装了
echo ""
echo ">>> [步骤 1/4] 检查当前用户 HOME 是否已安装 openclaw: $CURRENT_HOME"
if has_openclaw "$CURRENT_HOME"; then
    RUNTIME_USER="$CURRENT_USER"
    RUNTIME_HOME="$CURRENT_HOME"
    DETECT_SOURCE="current_user"
    echo "✓ 命中：当前用户 $CURRENT_USER 下已安装 openclaw"
else
    echo "× 未命中：当前用户 $CURRENT_USER 下未发现 openclaw"
fi
...
if [ -z "$RUNTIME_USER" ]; then
    ...
    for user_home in /home/*; do
        ...
        if has_openclaw "$user_home"; then
            ...
            echo "✓ 命中：用户 $user_name ($user_home) 下已安装 openclaw"
            break
        fi
    done
    shopt -u nullglob 2>/dev/null || true
    if [ -z "$RUNTIME_USER" ]; then
        echo "× 未命中：/home 下共探测 $SCANNED_COUNT 个用户，均未发现 openclaw"
    fi
else
    echo "（已在上一步命中，跳过）"
fi
...
if [ -z "$RUNTIME_USER" ] && [ "$CURRENT_USER" != "root" ]; then
    if has_openclaw "/root"; then
        RUNTIME_USER="root"
        RUNTIME_HOME="/root"
        DETECT_SOURCE="root"
        echo "✓ 命中：/root 下已安装 openclaw"
    else
        echo "× 未命中：/root 下未发现 openclaw"
    fi
else
...
if [ -z "$RUNTIME_USER" ]; then
    ...
    echo "⚠ 所有路径均未发现 openclaw，兜底返回当前用户 $CURRENT_USER（上层据此决定是否继续）"
else
