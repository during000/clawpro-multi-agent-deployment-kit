...
# ==================== 幂等检测：已安装则跳过 ====================
# 关键文件全部存在 → 认为安装完整，直接退出（节省 npm 下载 + install 时间）
HERMES_PLUGIN_DIR="$HERMES_AGENT_DIR/plugins/memory/memory_tencentdb"
ENVFILE="/etc/profile.d/memory-tencentdb-env.sh"

if [ -d "$TDAI_INSTALL_DIR/node_modules" ] && \
   [ -f "$TDAI_INSTALL_DIR/src/gateway/server.ts" ] && \
   [ -f "$TDAI_INSTALL_DIR/bin/read-local-memory.mjs" ] && \
   [ -L "$HERMES_PLUGIN_DIR" ] && [ -d "$HERMES_PLUGIN_DIR" ] && \
   [ -f "$ENVFILE" ]; then
    echo "[memory-tencentdb] Plugin already installed and complete, skipping reinstall"
    exit 0
fi

echo "[memory-tencentdb] Plugin not fully installed, proceeding with installation..."
