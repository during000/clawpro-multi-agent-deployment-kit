#!/usr/bin/env python3
"""
TC-PLUGIN-REDELIVER 企业插件重装后再次下发

验证修复点 controller/openclaw_reinstall.go::clearEnterpriseDistributionRecords：
实例重装时清空企业插件下发记录（PluginDistributionRecord），使重装后实例在管控端
插件下发页恢复为 uninstalled，企业插件可被再次下发，且 /admin/plugins/instances
仍能查到这台实例。

完整链路（单文件闭环，自建实例 + 自动销毁）：
    1. 创建实例（POST /openclaw/create）→ 等待 CVM running + Agent 服务就绪
    2. 管控端创建企业插件（POST /admin/plugins/create）
    3. 下发企业插件到实例（POST /admin/plugins/distribute）
         → /admin/plugins/instances 可查到实例，且 status 离开 uninstalled
         → 等待首次下发收敛（释放 plugin_distribute 分布式锁）
    4. 重装实例（POST /openclaw/reset）→ 走完整个重装生命周期并恢复正常运行
    5. 重装后核心校验：
         a. /admin/plugins/instances 仍能查到这台实例
         b. 该实例插件 status == uninstalled（下发记录已被 clearEnterpriseDistributionRecords 清空）
    6. 再次下发同一企业插件（POST /admin/plugins/distribute）
         → /admin/plugins/instances 可查到实例，status 重新离开 uninstalled（验证可再次下发）
    7. 销毁实例（POST /openclaw/delete）—— 无论成功失败都在 finally 中清理

说明：
    - 覆盖三个接口：/admin/plugins/create、/admin/plugins/distribute、/admin/plugins/instances。
    - 会真实触发 CVM 创建 / ResetInstance / 销毁，仅可在集成测试环境中运行。
    - 依赖 SMH 存储服务（企业插件 zip 存储），未启用则跳过。

使用方式：
    export API=http://134.175.254.166
    export ADMIN_TOKEN=xxx          # 种子管理员 Token（用于创建测试管理员/用户）
    python3 test_admin_plugin_redelivery_after_reinstall.py
"""
...
from helpers.admin_plugin import (
    admin_create_plugin,
    admin_distribute_plugin,
    admin_plugin_find_instance,
    wait_plugin_distributed,
    wait_plugin_settled,
    wait_plugin_instance_status,
)
...
def main():
    ...
    db_id = None
    failed = False
    slug = f"e2e-plugin-redeliver-{int(time.time())}"

    try:
        helpers.ensure_gateway_ui_enabled(admin.token)

        user = setup_user(admin.token, "ent-plugin-redeliver")
        client = user_client(user.token)

        # ── 步骤 1：创建实例并等待就绪 ──
        name = f"{config.INSTANCE_NAME_PREFIX}plugin-redeliver-{int(time.time())}"
        ...
        # ── 步骤 2：管控端创建企业插件 ──
        print(f">>> 步骤 2：创建企业插件 slug={slug} version={PLUGIN_VERSION} ...")
        resp = admin_create_plugin(admin.token, slug, PLUGIN_NAME, PLUGIN_VERSION)
        assert resp.status_code == 200, f"创建插件失败: {resp.status_code} {resp.text[:300]}"
        assert resp.json().get("ok"), f"创建插件响应异常: {resp.json()}"
        print("    企业插件创建成功 ✓")
        print()

        # ── 步骤 3：首次下发 + 校验实例可查到 ──
        print(">>> 步骤 3：首次下发企业插件到实例 ...")
        resp = admin_distribute_plugin(
            admin.token, slug, PLUGIN_VERSION,
            select_all=True, statuses=["uninstalled"], search=search,
        )
        assert resp.status_code == 200, f"下发失败: {resp.status_code} {resp.text[:300]}"
        assert resp.json().get("task_id"), f"下发响应缺少 task_id: {resp.json()}"
        print(f"    下发任务创建 ✓  task_id={resp.json().get('task_id')}")

        inst_item = wait_plugin_distributed(admin.token, slug, db_id, search=search)
        assert inst_item is not None, "首次下发后应能在 /admin/plugins/instances 查到实例"
        assert inst_item.get("status") != "uninstalled", \
            f"首次下发后 status 不应为 uninstalled，实际: {inst_item.get('status')}"
        print(f"    /admin/plugins/instances 可查到实例 ✓  status={inst_item.get('status')}")

        print(">>> 等待首次下发收敛（释放下发锁）...")
        settled = wait_plugin_settled(admin.token, slug, db_id, search=search)
        ...
        # ── 步骤 5：重装后核心校验 ──
        print(">>> 步骤 5：重装后校验 /admin/plugins/instances 仍可查到实例，且下发记录已清空 ...")
        # clearEnterpriseDistributionRecords 在 /openclaw/reset 处理内同步执行，
        # 这里短轮询以兼容 MySQL 主从/读写延迟。
        cleared = wait_plugin_instance_status(
            admin.token, slug, db_id, ("uninstalled",), timeout=120, search=search,
        )
        assert cleared is not None, "重装后应仍能在 /admin/plugins/instances 查到实例"
        assert cleared.get("status") == "uninstalled", \
            f"重装后下发记录应被清空（status=uninstalled），实际: {cleared.get('status')}"
        print(f"    重装后实例仍可查到且下发记录已清空 ✓  status={cleared.get('status')}")
        print()

        # ── 步骤 6：再次下发，验证企业插件可被重新下发 ──
        print(">>> 步骤 6：重装后再次下发企业插件 ...")
        resp = admin_distribute_plugin(admin.token, slug, PLUGIN_VERSION, [db_id])
        assert resp.status_code == 200, f"再次下发失败: {resp.status_code} {resp.text[:300]}"
        assert resp.json().get("task_id"), f"再次下发响应缺少 task_id: {resp.json()}"
        print(f"    再次下发任务创建 ✓  task_id={resp.json().get('task_id')}")

        redelivered = wait_plugin_distributed(admin.token, slug, db_id, search=search)
        assert redelivered is not None, "再次下发后应能在 /admin/plugins/instances 查到实例"
        assert redelivered.get("status") != "uninstalled", \
            f"再次下发后 status 不应为 uninstalled，实际: {redelivered.get('status')}"
        print(f"    重装后企业插件可再次下发 ✓  status={redelivered.get('status')}")
        print()

        print("=" * 60)
        print("  TC-PLUGIN-REDELIVER 企业插件重装后再次下发 — 全部通过 ✓")
        print("=" * 60)
        ...
        print()
        print("=" * 60)
        print("  TC-PLUGIN-REDELIVER 企业插件重装后再次下发 — 失败 ✗")
        print("=" * 60)
