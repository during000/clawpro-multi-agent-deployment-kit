#!/usr/bin/env python3
"""
TC-1.1 模型全生命周期：CRUD + 启用/禁用可见性联动
...
使用方式：
  ...
  python3 test_model_lifecycle.py
"""
...
def main():
    ...
    # ── Setup ──
    admin = setup_admin("model-lifecycle")
    user = setup_user(admin.token, "model-lifecycle", instance_quota=0)
    model_db_id = None

    try:
        model_name = f"IntTest Lifecycle ({config.MODEL_ID})"
        ...
        # 步骤 2：查询模型列表，记录 model_db_id（按唯一 model_name 定位）
        print(">>> 步骤 2：查询模型列表 ...")
        ...
        # 步骤 4：用户侧查询 → 模型可见（按 model_name 断言，避免并发时同 MODEL_ID 干扰）
        print(">>> 步骤 4：验证用户可见 ...")
