#!/usr/bin/env python3
"""
TC-SK-REDELIVER 企业技能重装后再次下发

验证修复点 controller/openclaw_reinstall.go::clearEnterpriseDistributionRecords：
实例重装时清空企业技能下发记录（SkillDistributionRecord），使重装后实例在管控端
技能下发页恢复为 uninstalled，企业技能可被再次下发，且 /admin/skills/instances
仍能查到这台实例。

完整链路（单文件闭环，自建实例 + 自动销毁）：
    1. 创建实例（POST /openclaw/create）→ 等待 CVM running + Agent 服务就绪
    2. 管控端创建企业技能（POST /admin/skills/create）
    3. 下发企业技能到实例（POST /admin/skills/distribute）
         → /admin/skills/instances 可查到实例，且 status 离开 uninstalled
         → 等待首次下发收敛（释放 skill_dist 分布式锁）
    4. 重装实例（POST /openclaw/reset）→ 走完整个重装生命周期并恢复正常运行
    5. 重装后核心校验：
         a. /admin/skills/instances 仍能查到这台实例
         b. 该实例技能 status == uninstalled（下发记录已被 clearEnterpriseDistributionRecords 清空）
    6. 再次下发同一企业技能（POST /admin/skills/distribute）
         → /admin/skills/instances 可查到实例，status 重新离开 uninstalled（验证可再次下发）
    7. 销毁实例（POST /openclaw/delete）—— 无论成功失败都在 finally 中清理

说明：
    - 覆盖三个接口：/admin/skills/create、/admin/skills/distribute、/admin/skills/instances。
    - 会真实触发 CVM 创建 / ResetInstance / 销毁，仅可在集成测试环境中运行。
    - 依赖 SMH 存储服务（企业技能 zip 存储），未启用则跳过。

使用方式：
    export API=http://134.175.254.166
    export ADMIN_TOKEN=xxx          # 种子管理员 Token（用于创建测试管理员/用户）
    python3 test_enterprise_skill_redelivery.py
"""
...
def main():
    ...
    db_id = None
    failed = False
    slug = f"e2e-redeliver-{int(time.time())}"

    try:
        helpers.ensure_gateway_ui_enabled(admin.token)

        user = setup_user(admin.token, "ent-skill-redeliver")
        client = user_client(user.token)

        # ── 步骤 1：创建实例并等待就绪 ──
        name = f"{config.INSTANCE_NAME_PREFIX}redeliver-{int(time.time())}"
        ...
        # ── 步骤 2：管控端创建企业技能 ──
        print(f">>> 步骤 2：创建企业技能 slug={slug} version={SKILL_VERSION} ...")
        resp = admin_create_skill(admin.token, slug, SKILL_NAME, SKILL_VERSION)
        assert resp.status_code == 200, f"创建技能失败: {resp.status_code} {resp.text[:300]}"
        assert resp.json().get("ok"), f"创建技能响应异常: {resp.json()}"
        print("    企业技能创建成功 ✓")
        print()

        # ── 步骤 3：首次下发 + 校验实例可查到 ──
        print(">>> 步骤 3：首次下发企业技能到实例 ...")
        resp = admin_distribute_skill(
            admin.token, slug, SKILL_VERSION,
        ...
        inst_item = wait_skill_distributed(admin.token, slug, db_id, search=search)
        assert inst_item is not None, "首次下发后应能在 /admin/skills/instances 查到实例"
        assert inst_item.get("status") != "uninstalled", \
            f"首次下发后 status 不应为 uninstalled，实际: {inst_item.get('status')}"
        print(f"    /admin/skills/instances 可查到实例 ✓  status={inst_item.get('status')}")

        print(">>> 等待首次下发收敛（释放下发锁）...")
        settled = wait_skill_settled(admin.token, slug, db_id, search=search)
        ...
        # ── 步骤 5：重装后核心校验 ──
        print(">>> 步骤 5：重装后校验 /admin/skills/instances 仍可查到实例，且下发记录已清空 ...")
        # clearEnterpriseDistributionRecords 在 /openclaw/reset 处理内同步执行，
        # 这里短轮询以兼容 MySQL 主从/读写延迟。
        cleared = wait_skill_instance_status(
            admin.token, slug, db_id, ("uninstalled",), timeout=120, search=search,
        )
        assert cleared is not None, "重装后应仍能在 /admin/skills/instances 查到实例"
        ...
        # ── 步骤 6：再次下发，验证企业技能可被重新下发 ──
        print(">>> 步骤 6：重装后再次下发企业技能 ...")
        resp = admin_distribute_skill(admin.token, slug, SKILL_VERSION, [db_id])
        ...
        redelivered = wait_skill_distributed(admin.token, slug, db_id, search=search)
        assert redelivered is not None, "再次下发后应能在 /admin/skills/instances 查到实例"
        assert redelivered.get("status") != "uninstalled", \
            f"再次下发后 status 不应为 uninstalled，实际: {redelivered.get('status')}"
        print(f"    重装后企业技能可再次下发 ✓  status={redelivered.get('status')}")
        print()

        print("=" * 60)
        print("  TC-SK-REDELIVER 企业技能重装后再次下发 — 全部通过 ✓")
        print("=" * 60)
        ...
        print()
        print("=" * 60)
        print("  TC-SK-REDELIVER 企业技能重装后再次下发 — 失败 ✗")
        print("=" * 60)
