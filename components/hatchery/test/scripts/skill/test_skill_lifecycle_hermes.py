#!/usr/bin/env python3
"""
TC-H5.1 Hermes 技能全生命周期：初始安装 + 手动安装 + 重试/取消

验证 hermes 实例技能的完整生命周期：
  1. 初始技能包自动安装 + 状态查询
...
使用方式：
  ...
  python3 test_hermes_skill_lifecycle.py
"""
...
def main():
    ...
    admin = setup_admin("hermes-skill-life")
    user = None
    inst = None
    ...
    try:
        ...
        user = setup_user(admin.token, "hermes-skill-life")
        inst = setup_hermes_instance(user.token, "skill-life")
        ...
        # 步骤 5：手动安装合法技能
        # Hermes 也支持 self-improving-agent 技能
        # 注意：与 openclaw 不同，hermes 已安装列表里 name 字段就等于 slug 本身，
        # 没有 slug→display name 的映射；所以预期 name = slug。
        INSTALL_SKILL = "self-improving-agent"
        INSTALL_SKILL_NAME = "self-improving-agent"
