#!/usr/bin/env python3
"""
集成测试：管理员重装普通用户实例（HandleAdminResetInstance）完整流程

完整链路（单文件内闭环，关注管控端重装核心流程）：
    1. 普通用户创建实例（POST /openclaw/create，使用 TOKEN）
    2. 等待 CVM 就绪（GET /openclaw/status → running）
    3. 等待 Agent 服务就绪（GET /openclaw/check-openclaw-port → running）
    4. 管理员触发重装（POST /admin/instances/reset，使用 ADMIN_TOKEN）
    ...
    6. 确认正常运行后，普通用户主动销毁实例（POST /openclaw/delete）

说明：
    - 本测试覆盖管控端重装（HandleAdminResetInstance → commonHandleResetInstance），
      与 test_reinstall_openclaw.py（用户自助重装）形成对照：实例归属普通用户，
      但由管理员通过 /admin/instances/reset 触发重装，验证管控端重装链路能跑通且
      重装后实例恢复正常。
    - 创建 / 状态查询 / 服务探活 / 销毁均以实例所有者（普通用户 TOKEN）身份进行，
      仅"触发重装"这一步以管理员（ADMIN_TOKEN）身份进行。
    - 公共的鉴权 / 参数校验等接口层面用例不在本测试范围内（由 Go 单元测试与
      test_admin_reset_approve_device.py 等覆盖），这里只验证"管控端重装核心流程
      能跑通且重装后实例恢复正常"。
    - 会真实触发 CVM ResetInstance，仅可在集成测试环境中运行。

环境变量：
    API          hatchery 服务地址
    TOKEN        普通用户 OpenAPI token（必填，形如 hk-xxx，实例所有者）
    ADMIN_TOKEN  管理员 token（必填，用于触发管控端重装）
"""
...
from helpers.api import health_check, ApiClient, seed, ADMIN_TOKEN

IDENTIFIER = os.environ.get("IDENTIFIER", "")
INSTANCE_NAME = f"admin-reinstall-{IDENTIFIER}-{int(time.time())}"
...
if not ADMIN_TOKEN:
    print("ERROR: please set environment variable ADMIN_TOKEN")
    print("  export ADMIN_TOKEN=xxx")
    sys.exit(1)

# 实例所有者：普通用户（用户侧接口带 X-OpenAPI）
client = ApiClient(TOKEN, openapi=True)


def create_instance():
    print(f">>> Create instance as user (name={INSTANCE_NAME}) ...")
...
def admin_reinstall_instance(instance_db_id):
    """
    管理员通过 POST /admin/instances/reset 触发重装。

    与用户自助重装（POST /openclaw/reset）不同，这里以管理员身份（ADMIN_TOKEN，
    即 seed 客户端）调用管控端接口，验证 HandleAdminResetInstance 链路。
    实例所有者仍是普通用户，但管理员有权对其触发重装。
    """
    print(f">>> Admin reinstall instance (id={instance_db_id}) ...")
    resp = seed.post("/admin/instances/reset", params={"id": str(instance_db_id)},
                     data={}, timeout=60, expect=None, raw=True)
    if resp.status_code != 200:
        print(f"    Admin reinstall failed: HTTP {resp.status_code}: {resp.text[:300]}")
        sys.exit(1)
    data = resp.json()
    if not data.get("ok"):
        print(f"    Admin reinstall failed: {data.get('error', data)}")
        sys.exit(1)
    print("    Admin reinstall request submitted (ok=true)")


def delete_instance(instance_db_id):
    print(f">>> Delete instance as user (id={instance_db_id}) ...")
...
def main():
    ...
    # 4. 管理员触发重装（管控端 /admin/instances/reset）
    admin_reinstall_instance(db_id)
    print()

    # 5. 销毁前置门禁：确认重装走完整个生命周期并恢复正常运行
    #    （先离开 running 进入过渡态，再回到稳定 running + Agent 服务 running）
    status_data, service_data = wait_for_reinstall_complete(db_id, stage="（管理员重装后）")
    print()

    print(">>> Final status (after admin reinstall):")
