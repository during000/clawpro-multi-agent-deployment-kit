...
import helpers
from helpers import verify_channel_configured, verify_qqbot_delivery
...
def main():
    ...
    try:
        ...
        model_ctx = setup_model(
            admin.token, model_id=config.MODEL_ID, model_name=f"IntTest CH QQ ({config.MODEL_ID})",
            api_key=config.MODEL_API_KEY, url=config.MODEL_URL,
        )
        user = setup_user(admin.token, "ch-qq")
        inst = setup_instance(user.token, "ch-qq")
        ...
        verify_channel_configured(inst, "qqbot", timeout=180)
        print("    QQ 通道可用性验证通过 ✓")

        print(">>> 步骤 3b：推送消息验证 ...")
        gw = connect_from_inst(inst, timeout=15)
        try:
            delivery = verify_qqbot_delivery(gw, verbose=True, timeout=60, channel_ready_timeout=90)
            if not delivery["success"]:
                print(f"    ⚠️ QQ 通道推送消息失败（跳过）: {delivery.get('error', '未知错误')}")
            else:
                print(f"    QQ 通道推送消息验证通过 ✓ (delivered={delivery['delivered']})")
        finally:
            gw.close()
