#!/usr/bin/env python3
"""
TC-H4.5 Hermes QQ 通道配置 + 查询确认

验证 hermes 实例的 QQ 通道凭证配置、查询确认、删除完整流程。
...
使用方式：
  ...
  export QQBOT_APP_ID=xxx  QQBOT_APP_SECRET=xxx
  python3 test_qq_channel_hermes.py
"""
...
def main():
    ...
    if not config.QQBOT_APP_ID or not config.QQBOT_APP_SECRET:
        print("错误: 未设置 QQBOT_APP_ID / QQBOT_APP_SECRET 环境变量")
        print("  通道测试需要真实凭证，不支持跳过。")
        sys.exit(1)

    require_model_config()
    print()
    admin = setup_admin("hermes-ch-qq")
    user = None
    inst = None
    model_ctx = None
    ...
    try:
        ...
        model_ctx = setup_model(
            ...
            model_name=f"IntTest Hermes CH QQ ({config.MODEL_ID})",
            api_key=config.MODEL_API_KEY, url=config.MODEL_URL,
        )
        user = setup_user(admin.token, "hermes-ch-qq")
        inst = setup_hermes_instance(user.token, "ch-qq")
        ...
        resp = retry_on_gateway_restart(
            lambda: helpers.user_set_channel(
                user.token, inst.db_id, "qqbot",
                keys=["app_id", "app_secret"],
                values=[config.QQBOT_APP_ID, config.QQBOT_APP_SECRET],
            )
        )
        assert resp.status_code == 200, f"配置 QQ 通道失败: status={resp.status_code}, body={resp.text}"
        print("    配置成功 ✓")
        time.sleep(10)
        ...
        assert "qqbot" in channels or any(
            c.get("channel_id", "") == "qqbot"
            for c in (channels if isinstance(channels, list) else [])
        ), f"QQ 通道应存在于配置中: {channels}"
        print("    查询确认 ✓")
        ...
        # 步骤 3b：软检查 hermes-gateway 是否真的把 qqbot 通道拉起来了
        print(">>> 步骤 3b：检查 hermes channel 连接状态 ...")
        expect_hermes_channel_connected(user.token, inst.db_id, min_enabled=1, timeout=90)

        # 步骤 4：删除配置
        print(">>> 步骤 4：删除 QQ 通道配置 ...")
        resp = retry_on_gateway_restart(
            lambda: helpers.user_del_channel(user.token, inst.db_id, "qqbot")
        )
        assert resp.status_code == 200, f"删除 qqbot 失败: status={resp.status_code}, body={resp.text}"
        print("    删除成功 ✓")
        time.sleep(5)
        ...
        for attempt in range(6):
            ...
            if isinstance(channels, dict):
                still_present = "qqbot" in channels and (
                    channels["qqbot"].get("enabled", False)
                    if isinstance(channels.get("qqbot"), dict)
                    else True
                )
            else:
                still_present = "qqbot" in str(channels)
            if not still_present:
                break
            time.sleep(5)
        assert not still_present, f"删除 qqbot 后仍存在于通道列表中: {channels}"
        print("    qqbot 已从列表移除 ✓")
